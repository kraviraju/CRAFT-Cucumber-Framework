...
public class LoginStepDefs extends MasterStepDefs {
	...
	public void application_should_stay_on_login_page() {
		currentScenario.embed(Util.takeScreenshot(driver.getWebDriver()),"image/png");
	...
	public void application_should_login_navigate_to_FlightFinder_page() throws InterruptedException {
		Thread.sleep(1500);
		currentScenario.embed(Util.takeScreenshot(driver.getWebDriver()),"image/png");
		assertTrue(driver.getTitle().contains("Find a Flight: Mercury Tours:"),"Find a Flight page displayed?");
	}
	
	
	@Then("^The application should not log me in and show login page")
	public void The_application_should_not_log_me_in_and_show_login_page() throws InterruptedException {
		Thread.sleep(1500);
		currentScenario.embed(Util.takeScreenshot(driver.getWebDriver()),"image/png");
		assertTrue(driver.getTitle().contains("Sign-on: Mercury Tours"),"Login page displayed?");
	}
}
